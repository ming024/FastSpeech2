...
- Inputs can be multihot phonological feature vectors instead of one-hot character/phone embeddings. See dissertation Gutkin et. al. 2018, Wells and Richmond 2021, and my dissertation). For this to work you must have a mapping to IPA. Panphon is used to generate 24 segmental features. If your language uses phonemic tone, please ammend features.py to use the 7 features from Wang 1967. Otherwise set the number of features accordingly in `config/*/model.py` and `config/*/preprocess.py`.
